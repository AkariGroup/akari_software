...
class PriorBox(object):
    def __init__(
        self,
        input_shape: Optional[Tuple[int, int]] = None,  # [in_w, in_h]
        output_shape: Optional[Tuple[int, int]] = None,  # [img_w, img_h]
    ...
    def decode(
        ...
        """Decodes the locations (x1, y1, x2, y2) and scores (c)
        from the priors, and the given loc and conf.
        Ignore low scores based on ignore_score.
        Args:
            loc (np.ndarray): loc produced from loc layers of shape
            [num_priors, 4]. '4' for [x_c, y_c, w, h].
            conf (np.ndarray): conf produced from conf layers of shape
            [num_priors, 2]. '2' for [p_non_face, p_face].
            iou (np.ndarray): iou produced from iou layers of shape
            [num_priors, 1]. '1' for [iou].
        ...
        Return:
            dets (np.ndarray): dets is concatenated by bboxes, landmarks and scores.
                bboxes consists of num_priors
                * (x1, y1, x2, y2) of shape [num_priors, 4].
                landmarks consists of num_priors
                * (x_le, y_le, x_re, y_r2, x_n, y_n, x_ml, y_ml, x_mr, y_mr)
