...
from akari_client.serial.feetech import FeetechController
...
from . import feetech_init
from ._error import serializer


class FeetechControllerServiceServicer(
    joints_controller_pb2_grpc.JointsControllerServiceServicer
):
    def __init__(
        self, config: JointManagerFeetechSerialConfig, joint_manager: JointManager
    ) -> None:
        self._config = config
    ...
    def _initialize_joints(self) -> Mapping[str, FeetechController]:
        ...
        joint_controllers = cast(List[FeetechController], jcs)

        feetech_init.initialize_joint_limit(
            joint_controllers,
            self._config,
        )
        feetech_init.initialize_default_velocity(
            joint_controllers,
            self._config,
        )
        feetech_init.initialize_default_acceleration(
            joint_controllers,
            self._config,
        )
